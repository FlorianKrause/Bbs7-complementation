...
genopheno <- read.table("fam1input_groups.txt", sep = " ", header = TRUE, na.strings=c(""), colClasses = c("character"))
...
anova(lm(genopheno[, "fat.lean70"]~genopheno[, "WG"])) #  Significant

genopheno$GenGood <- as.numeric(genopheno$GenGood)
...
lkup[df[, "ORDERED.GENOTYPES"]=="y/MDC"] <- "y/I8∆1"
lkup[df[, "ORDERED.GENOTYPES"]=="y/MDC"] <- "y/I8∆1"
...
nwo <- ordered(df$ORDERED.GENOTYPES, levels = c("BFMI/BFMI", "y/I8∆1", "x/B6N"))
